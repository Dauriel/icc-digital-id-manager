...
import pytest
from manager.tests.api_view_test_classes import TestView, returns_status_code_http_200_ok
...
class TestWebhooksConnectionsAPIView(TestWebhooksAPIView):
    ...
    def test_calls_credential_workflow(self, setup, dependency_mocks):
        with dependency_mocks as mocks:
            ...
            mocks["credential_offer_create"].assert_called_once_with("1", "mock connection invitation")
    ...
    def test_id_not_found(self, setup, dependency_mocks):
        ...
        with dependency_mocks as mocks:
            ...
            mocks["LOGGER"].assert_has_calls(
                [
                    call.info(
                        "webhook: received: topic: 'connections' - state: 'response' - message: {'state': 'response', 'connection_id': '7'}"
                    ),
    ...
    def test_invalid_connection_id(self, setup, dependency_mocks):
        ...
        with dependency_mocks as mocks:
            ...
            returns_status_code_http_200_ok(response)
            mocks["connection_invitation_accept"].assert_called_once_with("random test connection id")
            mocks["credential_offer_create"].assert_not_called()
            mocks["LOGGER"].assert_has_calls(
                [
                    call.info(
        ...
        with dependency_mocks as mocks:
            ...
            mocks["LOGGER"].assert_has_calls(
                [
                    ...
                    call.info("webhook: topic: connections and state: random test state is invalid"),
                ]
            )
    ...
    def test_malfomed_request(self, setup, dependency_mocks):
        ...
        with dependency_mocks as mocks:
            ...
            mocks["LOGGER"].assert_has_calls(
                [
                    call.info(
                        "webhook: received: topic: 'connections' - state: 'None' - message: {'malformed': 'penguin', 'connections': 'sarah, michael and jonny'}"
                    ),
    ...
    def test_connection_id_not_found(self, setup, dependency_mocks):
        ...
        with dependency_mocks as mocks:
            ...
            mocks["LOGGER"].assert_has_calls(
                [
                    call.info(
                        "webhook: received: topic: 'issue_credential' - state: 'credential_issued' - message: {'state': 'credential_issued', 'connection_id': '7'}"
                    ),
