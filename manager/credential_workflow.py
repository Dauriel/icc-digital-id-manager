...
def connection_invitation_accept(connection_id: str) -> ConnectionInvitation:
    try:
        ...
        LOGGER.error(
            f"connection_invitation_accept: connection_id: {connection_id} - error: {e}"
        )
...
def credential_offer_accept(connection_id: str) -> CredentialOffer:
    try:
        ...
        LOGGER.error(
            f"credential_offer_accept: connection_id: {connection_id} - error: {e}"
        )


def _step_accept(model_class, connection_id: str):
    model = (
        model_class.objects.filter(connection_id=connection_id)
        .order_by("-created")
        .first()
    )
    if not model:
        raise RuntimeError(
            f"Not found: {model_class} with connection_id: {connection_id}"
        )
    model.accepted = True
    model.save()
    return model
...
def credential_offer_create(
    ...
    aca_client = ACAClientFactory.create_client()
    aca_client.send_credential_offer(aca_credential_offer, connection_id)
